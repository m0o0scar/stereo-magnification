...
class MPI(object):
    ...
    def build_train_graph(self,
        ...
        with tf.name_scope('loss'):
            if which_loss == 'vgg':
                ...
                p1 = compute_error(
                    vgg_real['conv1_2'], vgg_fake['conv1_2']) / 2.6
                p2 = compute_error(
                    vgg_real['conv2_2'], vgg_fake['conv2_2']) / 4.8
                p3 = compute_error(
                    vgg_real['conv3_2'], vgg_fake['conv3_2']) / 3.7
                p4 = compute_error(
                    vgg_real['conv4_2'], vgg_fake['conv4_2']) / 5.6
                p5 = compute_error(
                    vgg_real['conv5_2'], vgg_fake['conv5_2']) * 10 / 1.5
                total_loss = p0 + p1 + p2 + p3 + p4 + p5

            if which_loss == 'pixel':
                tgt_image = self.preprocess_image(raw_tgt_image)
                total_loss = tf.reduce_mean(
                    tf.nn.l2_loss(output_image - tgt_image))
    ...
    def format_network_input(self, ref_image, psv_src_images, ref_pose,
        ...
        for i in range(num_psv_source):
            curr_pose = tf.matmul(
                psv_src_poses[:, i], tf.matrix_inverse(ref_pose))
