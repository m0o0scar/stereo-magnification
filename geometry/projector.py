...
def projective_forward_homography(src_images, intrinsics, pose, depths):
    ...
    proj_src_images = planar_transform(
...
def projective_inverse_warp(
    ...
    src_pixel_coords_shape = tf.shape(src_pixel_coords)
    src_pixel_coords_height = src_pixel_coords_shape[1]
    src_pixel_coords_width = src_pixel_coords_shape[2]
    output_img = tf.image.resize(
        img, [src_pixel_coords_height, src_pixel_coords_width], method=tf.image.ResizeMethod.BILINEAR)
    if ret_flows:
